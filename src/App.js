...
function App() {
  ...
  const getDropdowns = data => {
    ...
    data.forEach(comment => {
      ...
      comment.fields.forEach((el,i) => {
        if (el.name === "whichSectionOfTheRouteAreYouCommentingOn") {
          nameIndex = i;
        }
      })
  ...
  const doUpdateURL = url => {
    const urlOverride = 'https://northeastgateway.commonplace.is/proposals/have-your-say'
    setCommonplaceURL(urlOverride);
    setLoading(true);
    const urls = urlOverride.split('commonplace.is');
    ...
    console.log(proxyurl + urlOverride)
    fetch(proxyurl + urlOverride)
